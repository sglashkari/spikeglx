...
%% Read data from file
[binaryFile,path] = uigetfile('D:\NeuralData\*.ap.bin', 'Select a Binary Files');
...
selparentpath = uigetdir('D:\Analysis','Select the main Directory for Saving Pos.p Files');
...
answer=questdlg('Is the tracking time in CSV in Neuropixels time?', 'Warning!', 'Yes', 'No', 'No');

thisFilePath = pwd;
cd(selparentpath);
paramspath = {dir('**/*.parms').folder};
paramsfilename = {dir('**/*.parms').name};
cd(thisFilePath);
